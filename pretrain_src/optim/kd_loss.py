...
def mse_loss(s_inputs, t_inputs, t_sample_weights=None, **kwargs):
    if t_sample_weights is None:
        loss = (s_inputs - t_inputs) ** 2
        return loss.mean()
    else:
        ...
        if per_sample_loss.shape[0] == t_sample_weights.shape[0]:
            ...
            weighted_loss = per_sample_loss * t_sample_weights
            return weighted_loss.mean()
        else:
            return per_sample_loss.mean()

def kd_loss(student_logits, teacher_logits, temperature=1, epsilon=1e-6, t_sample_weights=None, **kwargs):
    # typical KD loss
