...
def worker_process(args):
    ft_file, keys, img_ft_size, return_dict = args
    feature_store = {}
    with h5py.File(ft_file, 'r') as f:
        for key in keys:
            ft = f[key][...][:, :img_ft_size].astype(np.float32)
            feature_store[key] = ft
    return_dict.update(feature_store)

def read_hm3d_img_from_lmdb(ft_file, img_ft_size=768):
    data_env = lmdb.open(ft_file, readonly=True, lock=False)
    feature_store = {}

    with data_env.begin() as txn:
        with txn.cursor() as cursor:
            for key, value in cursor:
                key_str = key.decode()
                ft = msgpack.unpackb(value)
                ft = ft[:, :img_ft_size]
                if not ft.flags.writeable:
                    ft = np.copy(ft)
                feature_store[key_str] = ft
        
    data_env.close()
    return feature_store

def read_hm3d_obj_from_lmdb(ft_file, cat_mapping, category_number, obj_ft_size=768, max_objects=50):
    data_env = lmdb.open(ft_file)
    ...
    with data_env.begin() as txn:
        with txn.cursor() as cursor:
            for key, value in cursor:
                key_str = key.decode()
                obj_attrs = {}
                obj_fts = np.zeros((0, obj_ft_size), dtype=np.float32)
                
                if value is not None:
                    d = msgpack.unpackb(value)
                    obj_fts = d['fts'][:max_objects, :obj_ft_size]
                    
                    if not obj_fts.flags.writeable: # Ensure the numpy array is writeable before converting to a tensor
                        obj_fts = np.copy(obj_fts)  # Make a copy
                    ...
                    for attr_key, attr_value in d.items():
                        ...
                        if attr_key == 'centers':
                            attr_value_np = np.array(attr_value[:max_objects]).astype(np.float32)
                            if not attr_value_np.flags.writeable:
                                attr_value_np = np.copy(attr_value_np)  # Make a copy
                            obj_attrs['directions'] = attr_value_np
                        if attr_key == 'bboxes':
                            attr_value_np = np.array(attr_value[:max_objects]).astype(np.float32)[:, 2:]
                            if not attr_value_np.flags.writeable:
                                attr_value_np = np.copy(attr_value_np)  # Make a copy
                            obj_attrs['sizes'] = torch.from_numpy(attr_value_np)
                        if attr_key == 'obj_names':
                            for i in range(len(attr_value[:max_objects])):
                                _, attr_value[i] = preprocess_name(attr_value[i],cat_mapping,category_number)
                            obj_attrs['names'] = attr_value[:max_objects]

                feature_store[key_str] = (obj_fts, obj_attrs)
    data_env.close()
    return feature_store
    
def read_img_features_from_h5py_multiprocess(ft_file, img_ft_size=768, num_processes=4):
    # Use a manager to create a shared dictionary
    manager = Manager()
    return_dict = manager.dict()

    # Open the file once to get the keys (to avoid opening it multiple times)
    with h5py.File(ft_file, 'r') as f:
        keys = list(f.keys())

    # Split keys into chunks for each process
    keys_per_process = np.array_split(keys, num_processes)

    # Prepare arguments for each process
    args = [(ft_file, key_chunk, img_ft_size, return_dict) for key_chunk in keys_per_process]

    # Create a pool of processes and distribute the work
    with Pool(processes=num_processes) as pool:
        pool.map(worker_process, args)
