...
def set_cuda(opts) -> Tuple[bool, int, torch.device]:
    ...
    if opts.local_rank != -1:
        ...
        rank, world_size = init_distributed(opts)
    ...
    return default_gpu, n_gpu, device, device_id


def wrap_model(
    model: torch.nn.Module, device: torch.device, local_rank: int, find_unused_parameters=True
) -> torch.nn.Module:
    model.to(device)
    ...
    if local_rank != -1:
        ...
        LOGGER.info(f"Use DDP to wrap the model to device_id: {local_rank}")
