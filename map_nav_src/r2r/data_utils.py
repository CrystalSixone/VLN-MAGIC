...
def load_data_from_file(args):
    anno_dir, dataset, split = args
    new_data = []
    if "rxr" in dataset:
        # RXR
        if "/" not in split:
            filepath = os.path.join(anno_dir, 'RxR', f'{dataset}_{split}_guide.jsonl')
            with jsonlines.open(filepath, 'r') as f:
                for item in f:
                    new_data.append(item)
        else:
            print(f'\nLoading augmented data {os.path.basename(split)} for pretraining...')
        ...
        if "/" not in split:  # the official splits
            filepath = os.path.join(anno_dir, f'{dataset.upper()}_{split}_roberta_enc.json')
...
def load_instr_datasets(anno_dir, dataset, splits, num_workers=6):
    args = [(anno_dir, dataset, split) for split in splits]
    with Pool(num_workers) as p:
        data = sum(p.map(load_data_from_file, args), [])
    return data

def construct_instrs(anno_dir, dataset, splits, tokenizer, max_instr_len=512, for_debug=False, tok=None, is_rxr=False, num_workers=6):
    data = []
    for i, item in enumerate(load_instr_datasets(anno_dir, dataset, splits, num_workers)):
        if is_rxr:
