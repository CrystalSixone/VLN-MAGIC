...
class EnvBatch(object):
    ...
    def __init__(self, args, connectivity_dir, scan_data_dir=None, feat_db=None, batch_size=100):
        ...
        if scan_data_dir:
            self.sims.setDatasetPath(scan_data_dir)
        self.sims.setNavGraphPath(connectivity_dir)
        self.sims.setRenderingEnabled(False)
        self.sims.setDiscretizedViewingAngles(True)   # Set increment/decrement to 30 degree. (otherwise by radians)
        self.sims.setCameraResolution(self.image_w, self.image_h)
        self.sims.setCameraVFOV(math.radians(self.vfov))
        self.sims.setBatchSize(self.args.batch_size) # Parallel initialization
    ...
    def getStates(self):
        ...
        """
        feature_states = []
        states = self.sims.getState()
        for i, state in enumerate(states):
            if self.aug_feat_db is not None:
                feature = self.aug_feat_db[int(i%(len(self.aug_feat_db)))].get_image_feature(state.scanId, state.location.viewpointId, self.args.img_type)
            else:
    ...
    def makeActions(self, actions):
        ...
        indexs, headings, elevations = [], [], []
        for (index, heading, elevation) in actions:
            indexs.append(index)
            headings.append(heading)
            elevations.append(elevation)
        
        self.sims.makeAction(indexs, headings, elevations)
