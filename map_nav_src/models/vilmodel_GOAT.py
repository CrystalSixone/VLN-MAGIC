...
class LanguageEncoderDo(nn.Module):
    ...
    def forward(self, txt_embeds, txt_masks, z_direc_embeds=None, z_direc_pzs=None, z_landm_embeds=None, z_landm_pzs=None,
        ...
        # Causal Intervention
        if z_direc_embeds is not None and (self.config.do_back_txt or self.config.do_front_txt):
...
class CausalImageEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.img_dropout = nn.Dropout(config.img_dropout)
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if self.config.name in ['REVERIE', 'SOON'] and self.config.obj_feat_size > 0:
        ...
        self.kdl = config.train_kdl
        if self.role == 'student' and self.kdl:
            ...
            self.kdl_img_w = nn.Linear(self.config.hidden_size, self.config.teacher_hidden_size)
            self.kdl_avg_img_w = nn.Linear(self.config.hidden_size, self.config.teacher_hidden_size)
            
            if config.kdl_adaptive_ability_weight:
                # let network to learn and adjust the weights for different losses
                if config.kdl_adaptive_ability_weight_type == 'learn_weight':
        ...
        if self.role == 'teacher' and self.config.train_kdl_teacher:
            self.txt_emb_w = nn.Linear(self.config.hidden_size, self.config.student_hidden_size)
            self.vp_txt_w = nn.Linear(self.config.hidden_size, self.config.student_hidden_size)
            self.gmap_txt_w = nn.Linear(self.config.hidden_size, self.config.student_hidden_size)
            self.local_cross_w = nn.Linear(self.config.hidden_size, self.config.student_hidden_size)
            self.global_cross_w = nn.Linear(self.config.hidden_size, self.config.student_hidden_size)
            self.kdl_img_w = nn.Linear(self.config.hidden_size, self.config.student_hidden_size)
            self.kdl_avg_img_w = nn.Linear(self.config.hidden_size, self.config.student_hidden_size)
    ...
    def forward_panorama_do_per_step(self,view_img_fts, loc_fts, nav_types, view_lens,
        ...
        if self.config.name not in ['REVERIE', 'SOON']:
            ...
            view_img_output = self.img_embeddings.img_self_encoder(
                view_img_embeds, src_key_padding_mask=img_masks.logical_not()
            )
            view_img_embeds, view_img_attns = view_img_output[0], view_img_output[1]
        ...
        if self.config.name == 'REVERIE' or self.config.name == 'SOON':
            ...
            img_masks = gen_seq_masks(view_lens)
            view_img_output = self.img_embeddings.pano_encoder(
        ...
        if self.config.adaptive_pano_fusion:
            ...
            traj_fused_embeded_update = torch.mul(traj_ori_embeds,traj_fused_weight_act)
            traj_fused_embeds = torch.sum(traj_fused_embeded_update,dim=1)
            return view_img_embeds, img_masks, traj_fused_embeds, view_img_attns
        else:
            return view_img_embeds, img_masks, None, view_img_attns
    ...
    def forward_navigation_per_step(
        ...
        front_vp_feats=None, front_gmap_feats=None, gflops_compute=False
    ):  
        ...
        gmap_embeds, gmap_attns = self.global_encoder.encoder(
        ...
        vp_embeds, vp_attns = self.local_encoder.encoder(vp_embeds, vp_masks, txt_embeds, txt_masks) 
    ...
    def forward_cfp_features(
        self, txt_embeds, txt_masks, gmap_img_embeds, gmap_step_ids, gmap_pos_fts, 
        gmap_masks, gmap_pair_dists, gmap_visited_masks, gmap_vpids,
        vp_img_embeds, vp_pos_fts, vp_masks, vp_nav_masks, vp_obj_masks, vp_cand_vpids,
        vp_obj_embeds=None, vp_obj_lens=None,
    ):  
        batch_size = txt_embeds.size(0)
        ...
        gmap_embeds = self.global_encoder.tim_self_encoder(
            gmap_embeds, gmap_masks
        )
        ...
        vp_embeds = vp_img_embeds + self.local_encoder.vp_pos_embeddings(vp_pos_fts)
        if self.config.name == 'REVERIE' or self.config.name == 'SOON':
            vp_embeds = self.local_encoder.tim_self_encoder(vp_embeds, vp_nav_masks)
        else:
            vp_embeds = self.local_encoder.tim_self_encoder(vp_embeds, vp_masks)
        ...
        return txt_outputs, vp_outputs, gmap_outputs
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            ...
            pano_embeds, pano_masks, pano_fused_embeds, pano_attns = self.forward_panorama_do_per_step(
            ...
            return pano_embeds, pano_masks, pano_fused_embeds, pano_attns
             ...
             return self.forward_navigation_per_step(
                ...
                batch['gflops_compute']
            )

        elif mode == 'instr_zdict_update':
            txt_embeds, txt_attns = self.forward_text(batch['z_txt'], batch['z_txt_mask'],
            ...
            txt_embeds, txt_attns = self.forward_text(batch['txt_ids'], batch['txt_masks']
                )
            split_traj_embeds, split_traj_vp_lens, split_traj_fused_embeds = self.img_embeddings(
                ...
                batch['traj_reverie_obj_fts'], batch['traj_reverie_obj_lens']
            )
            ...
            # Compute attention and aggragation, the same as pre-training
