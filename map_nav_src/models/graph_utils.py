...
class GraphMap(object):
    def __init__(self, start_vp):
        ...
        self.t_node_embeds = {}     # for teacher in knowledge distillation
    ...
    def update_node_embed(self, vp, embed, rewrite=False, teacher=False):
        if teacher:
            if rewrite:
                self.t_node_embeds[vp] = [embed, 1]
            else:
                if vp in self.t_node_embeds:
                    self.t_node_embeds[vp][0] += embed
                    self.t_node_embeds[vp][1] += 1
                else:
                    self.t_node_embeds[vp] = [embed, 1]
        else:
            if rewrite:
    ...
    def get_node_embed(self, vp, teacher=False):
        if teacher:
            return self.t_node_embeds[vp][0] / self.t_node_embeds[vp][1]
        else:
